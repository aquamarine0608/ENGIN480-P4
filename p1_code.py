...
class VineyardWind(UniformWeibullSite):
    def __init__(self, ti=0.1, shear=None):
        f = np.array([560.41, 419.61, 480.66, 638.61, 625.46, 667.67, 1029.07, 1282.98, 1340.08, 1123.80, 969.42, 862.23]) #Line 5
...
class RevolutionSouthFork(UniformWeibullSite):
    def __init__(self, ti=0.1, shear=None):
        f = np.array([652.94, 745.53, 622.32, 588.86, 474.39, 456.32, 717.71, 1225.30, 1385.41, 1037.11, 1158.19, 935.93]) #Line 5
...
class CoastalVirgina(UniformWeibullSite):
    def __init__(self, ti=0.1, shear=None):
        f = np.array([919.38, 990.99, 908.17, 525.05, 482.52, 572.45, 1149.10, 1424.91, 930.86, 506.00, 646.52, 944.05]) #Line 5
...
#here we import the turbine and site
windTurbines = SG_14222DD #edit
site = CoastalVirgina() #edit

noj = NOJ(site,windTurbines)

xvine, yvine = get_coords("vineyard.geojson", 32619)
xrevsfk, yrevsfk = get_coords("revolution_southfork.geojson", 32618)
xcoast, ycoast = get_coords("coastal_virginia.geojson", 32618)
xsofia, ysofia = get_coords("sofia.geojson", 32631)


simulationResult = noj(xcoast, ycoast) #edit
...
windTurbines.plot(xcoast,ycoast) #edit
c =plt.scatter(xcoast, ycoast, c=aep.sum(['wd','ws'])) #edit
